...
import org.jivesoftware.launcher.Constants;
import org.jivesoftware.launcher.UserInfo;
...
/*
    ...
    protected XMPPTCPConnectionConfiguration retrieveConnectionConfiguration() {
        ...
        //System.out.println("loginServer: " + loginServer);
...
//        }
     ...
     */
    private final class LoginPanel extends JPanel implements KeyListener, ActionListener, FocusListener, CallbackHandler, AuthorizationNotifier {
        ...
        LoginPanel() {
           ...
           // headerLabel.setFont(headerLabel.getFont().deriveFont(Font.BOLD));
            headerLabel.setFont(new Font("微软雅黑", Font.BOLD, 13));
            accountLabel.setText("Account:");
            ssoServerLabel.setText("Server:");
            accountNameLabel.setFont(new Font("微软雅黑", Font.BOLD, 13));
            serverNameLabel.setFont(new Font("微软雅黑", Font.BOLD, 13));
 ...
 //           usernameField.setText(UserInfo.getInstance().userID);
            ...
            add(passwordField,
                    new GridBagConstraints(1, 1, 2, 1,
                            1.0, 0.0, GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
                            new Insets(5, 5, 0, 0), 0, 0));
//            passwordField.setText(UserInfo.getInstance().userPWD);
...
//            serverField.setText(UserInfo.getInstance().getOpenFireServer());
            ...
            localPref.setAcceptAllCertificates( true );
            localPref.setDisableHostnameVerification( true );
		...
		if (f.getName().contains("@")) {
            ...
            // Handle arguments
            String username = Spark.getArgumentValue("userId");
             ...
             * add notifier
             * */
            NotifierManager.getInstance().addNotify(Constants.LOGIN_NOTIFIER_NAME, this);
            //add 
            
            new Thread("get authorization"){
        		
        		public void run(){
        			
        				UserInfo ui = UserInfo.getInstance();
						ui.setUI(UserInfo.getInstance().userID, UserInfo.getInstance().userPWD);
        				System.out.println(UserInfo.getInstance().userID);
        				System.out.println(UserInfo.getInstance().userPWD);
        				usernameField.setText(UserInfo.getInstance().userID);
        	        	passwordField.setText(UserInfo.getInstance().userPWD);
        	        	serverField.setText(UserInfo.getInstance().getOpenFireServer());
        	        	System.out.println(usernameField.getText()); 
        	        	System.out.println(passwordField.getPassword());
        	        	System.out.println(serverField.getText());
        	        	
        	        	validateLogin();
        			}                	                	                	                	
                	//Timeout
                	/*ui.user_status = Constants.CONNECT_EXCEPTION_TIMEOUT;
                	NotifierManager.getInstance().notifyITSth(Constants.LOGIN_NOTIFIER_NAME
                		, "Time out when connecting to server, Try again later");*/
        ...
        private String getBareJid()
        {
           ...
           // return Spark.getArgumentValue("username");
        }
...
*/            else if (e.getSource() == loginButton) {
        ...
        private void enableComponents(boolean editable) {
            ...
            usernameField.setEnabled(true);			//Changed by Jesson LIU from "editable" to "true"

            passwordField.setEditable(editable);
            passwordField.setEnabled(true);			//Changed by Jesson LIU from "editable" to "true"
            ...
            final SwingWorker loginValidationThread = new SwingWorker() {
                public Object construct() {
                    ...
                    if (loginSuccessfull) {
                        ...
                        EventQueue.invokeLater( () -> {
                            savePasswordBox.setEnabled(false);		//changed by Jesson LIU from "true" to "false"
                            autoLoginBox.setEnabled(false);			//changed by Jesson LIU from "true" to "false"
                            loginAsInvisibleBox.setVisible(true);
                            enableComponents(false);				//changed by Jesson LIU from "true" to "false"
                            setProgressBarVisible(false);
                            loginButton.setEnabled(true);			//added by Jesson LIU
                        } );
...
//            }
	...
	public void handle(Callback[] callbacks) throws IOException  {
		...
		@Override
		//@author Jesson LIU
		public void notifyMeSth(String msg){
			
			if(UserInfo.getInstance().user_status == Constants.LOGIN_EXCEPTION_NON_USER){
				//User do NOT exist in the system, a new user
				//Dialog for some information, this is just a simple notification dialog
				JOptionPane.showMessageDialog(loginDialog, msg/*"You are a new user, register yourself first"*/, "Notice",
...
/*				
                ...
                if (createAccountPanel.isRegistered()) {
                    ...
                    //loginButton.setEnabled(true);
                    //setProgressBarVisible(true);
                    //TODO delete it when the version is ready
                    validateLogin();
                }
                */
			}else if(UserInfo.getInstance().user_status == Constants.CONNECT_EXCEPTION_TIMEOUT){
				//connecting to server time out
				//Dialog for some information, this is just a simple notification dialog
				JOptionPane.showMessageDialog(loginDialog, msg/*"Time out when connecting to server"*/, "Error",
						JOptionPane.ERROR_MESSAGE);
				//TODO other operations
				System.exit(0);
			}
